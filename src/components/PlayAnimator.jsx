...
const Court = ({ children }) => (
  <svg
    viewBox="0 0 100 94"
    className="w-full rounded-2xl shadow ring-1 ring-white/10 bg-emerald-900/10"
  >
  ...
  <g>
    <circle
      ...
      className={`fill-white/90 ${
        active ? "stroke-emerald-400" : "stroke-white/40"
      }`}
    />
    <text
      x={x}
...
const Arrow = ({ from, to }) => {
  ...
  return (
    <g transform={`translate(${from.x} ${from.y}) rotate(${angle})`}>
      ...
      />
      <polygon
        points={`${len - 3},0 ${len - 5},1.8 ${len - 5},-1.8`}
...
export default function PlayAnimator({ play }) {
  ...
  const playersById = useMemo(() => {
    return Object.fromEntries((frame.players || []).map((p) => [p.id, p]));
  }, [frame]);
  ...
  return (
    <div className="grid gap-4">
      ...
      <p className="text-xs text-zinc-500">
        Step {idx + 1} / {steps.length} — Space/→ advance, ← back, R reset. Voice:
        “next/back/reset”.
      </p>
    </div>
  );
